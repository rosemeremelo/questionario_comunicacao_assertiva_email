...
const scoring = [[1,2,3],[1,2,3],[1,2,3],[1,2,3],[3,2,1],[3,2,1],[3,2,1],[1,2,3],[1,2,3],[1,2,3],[3,2,1],[1,2,3],[1,2,3],[1,2,3],[1,2,3],[3,2,1],[1,2,3],[1,2,3],[1,2,3],[3,2,1],[3,2,1],[1,2,3],[1,2,3],[3,2,1],[1,2,3],[3,2,1],[3,2,1],[1,2,3],[3,2,1],[3,2,1],[1,2,3],[3,2,1],[1,2,3],[1,2,3],[1,2,3],[1,2,3]];
...
window.setAns = (q, a) => {
    ...
    [0,1,2].forEach(i => document.getElementById(`b-${q}-${i}`).className = 'opt-btn' + (i===a ? ' '+cls[i] : ''));
    
    const done = answers.filter(x => x !== null).length;
    document.getElementById('progress-text').innerText = `${done} de 36 respondidas`;
    document.getElementById('progress-fill').style.width = `${(done/36)*100}%`;
};
